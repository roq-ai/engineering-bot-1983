...
import { deleteRoadmapById } from 'apiSdk/roadmaps';
...
function CompanyViewPage() {
  ...
  const roadmapHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('roadmap', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/roadmaps/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.roadmap?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/roadmaps/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
